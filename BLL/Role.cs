...
namespace DriveMgr.BLL
{
    ...
    public class Role
    {
        private static readonly DriveMgr.IDAL.IRole dal = DriveMgr.DALFactory.Factory.GetRoleDAL();
        ...
        {
            return DriveMgr.Common.JsonHelper.ToJson(dal.GetAllRole(where));
        }
        ...
        public bool Authorize(int roleId, string roleMenuButtonId)
        {
            ...
            List<DriveMgr.Model.RoleMenuButton> role_menu_button_addlist = new List<Model.RoleMenuButton>();    //需要新添加的（用户新勾选的）
            List<DriveMgr.Model.RoleMenuButton> role_menu_button_deletelist = new List<Model.RoleMenuButton>();   //需要删除的（用户去除勾选的）
            ...
            for (int i = 0; i < dt_role_menu_button_old.Rows.Count; i++)
            {
                ...
                {
                    DriveMgr.Model.RoleMenuButton roleMenuButton = new Model.RoleMenuButton();
                ...
                for (int i = 0; i < role_menu_button_userselect.Length; i++)
                {
                    ...
                    if (dt_role_menu_button_old.Select(string.Format("MenuId={0} and ButtonId={1}", menuId, buttonId)).Length == 0)
                    {
                        ...
                        if (parentId != -1 && !listParentMenuId.Contains(parentId))
                        {
                            ...
                            DriveMgr.Model.RoleMenuButton roleParentMenu = new Model.RoleMenuButton();
        ...
        {
            if (DriveMgr.Common.SqlInjection.GetString(order))   //简单的sql注入过滤
                order = "AddDate asc";
            int totalCount = dal.GetRoleUserCount(roleId);
            DataTable dt = dal.GetPagerRoleUser(roleId, order, pageSize, pageIndex);

            string strjson = DriveMgr.Common.JsonHelper.ToJson(dt);
