...
namespace DriveMgr.BLL
{
    ...
    {
        private static readonly DriveMgr.IDAL.IDepartment dal = DriveMgr.DALFactory.Factory.GetDepartmentDAL();
        ...
        {
            if (DriveMgr.Common.SqlInjection.GetString(departmentIds))   //简单sql防注入
                departmentIds = "";
            if (DriveMgr.Common.SqlInjection.GetString(order))
                order = "AddDate asc";
