            ...
            try
            {
                ...
                switch (action)
                {
                    case "search":
                        ...
                        if (ui_operatelog_username.Trim() != "" && !DriveMgr.Common.SqlInjection.GetString(ui_operatelog_username))   //防止sql注入
                            strWhere += string.Format(" and UserName like '%{0}%'", ui_operatelog_username.Trim());
                        if (ui_operatelog_userip.Trim() != "" && !DriveMgr.Common.SqlInjection.GetString(ui_operatelog_userip))
                            strWhere += string.Format(" and UserIp like '%{0}%'", ui_operatelog_userip.Trim());
                        if (ui_operatelog_info.Trim() != "" && !DriveMgr.Common.SqlInjection.GetString(ui_operatelog_info))
                            strWhere += string.Format(" and OperateInfo like '%{0}%'", ui_operatelog_info.Trim());
                        if (ui_operatelog_description.Trim() != "" && !DriveMgr.Common.SqlInjection.GetString(ui_operatelog_description))
                        ...
                        int totalCount;   //输出参数
                        string strJson = new DriveMgr.BLL.UserOperateLog().GetPager("tbUserOperateLog", "Id,UserName,UserIp,OperateInfo,Description,IfSuccess,OperateDate", sort + " " + order, pagesize, pageindex, strWhere, out totalCount);
