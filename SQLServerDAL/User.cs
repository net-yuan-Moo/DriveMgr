...
namespace DriveMgr.SQLServerDAL
{
    ...
    /// 用户（SQL Server数据库实现）
    /// </summary>
    public class User : DriveMgr.IDAL.IUser
    {
        /// <summary>
        /// 根据用户id获取用户
        /// </summary>
        ...
        public bool InitUserPwd(Model.User user)
        {
            ...
            object obj = DriveMgr.Common.SqlHelper.ExecuteNonQuery(DriveMgr.Common.SqlHelper.connStr, CommandType.Text, sql, paras);
        ...
        public DataTable GetUserInfo(int userId)
        {
            ...
            return DriveMgr.Common.SqlHelper.GetDataTable(DriveMgr.Common.SqlHelper.connStr, CommandType.Text, strSql.ToString(), new SqlParameter("@userId", userId));
        }
