...
{
    public class SiteRentalDAL:ISiteRentalDAL
    {
        /// <summary>
        /// 增加一条数据
        /// </summary>
        public bool AddSiteRental(SiteRentalModel model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into tb_SiteRental(");
            strSql.Append("VehicleId,RentObject,PriceConfigID,Longer,TotalPrice,Remark,CreatePerson,CreateDate,UpdatePerson,UpdateDate,DeleteMark,RentDate)");
            strSql.Append(" values (");
            strSql.Append("@VehicleId,@RentObject,@PriceConfigID,@Longer,@TotalPrice,@Remark,@CreatePerson,@CreateDate,@UpdatePerson,@UpdateDate,@DeleteMark,@RentDate)");

            SqlParameter[] parameters = {
					new SqlParameter("@VehicleId", SqlDbType.Int,4),
					new SqlParameter("@RentObject", SqlDbType.VarChar,100),
            ...
            parameters[1].Value = model.RentObject;
        ...
        /// 更新一条数据
        /// </summary>
        public bool UpdateSiteRental(SiteRentalModel model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update tb_SiteRental set ");
            strSql.Append("VehicleId=@VehicleId,");
            strSql.Append("RentObject=@RentObject,");
        ...
        public bool DeleteSiteRental(int Id)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("Update tb_SiteRental Set DeleteMark = 1 ");
            strSql.Append(" where Id=@Id");
            SqlParameter[] parameters = {
					new SqlParameter("@Id", SqlDbType.Int,4)
			};
        ...
        /// 得到一个对象实体
        /// </summary>
        public SiteRentalModel GetSiteRentalModel(int Id)
        {

            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 Id,VehicleId,RentObject,RentDate,PriceConfigID,Longer,TotalPrice,Remark,CreatePerson,CreateDate,UpdatePerson,UpdateDate,DeleteMark from tb_SiteRental ");
            strSql.Append(" where Id=@Id");
            SqlParameter[] parameters = {
					new SqlParameter("@Id", SqlDbType.Int,4)
			};
            ...
            SiteRentalModel model = new SiteRentalModel();
        ...
        /// 得到一个对象实体
        /// </summary>
        private SiteRentalModel DataRowToModel(DataRow row)
        {
            SiteRentalModel model = new SiteRentalModel();
            if (row != null)
            {
                ...
                if (row["RentObject"] != null)
                {
                    model.RentObject = row["RentObject"].ToString();
                }
