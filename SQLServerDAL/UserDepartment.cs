...
namespace DriveMgr.SQLServerDAL
{
    ...
    /// </summary>
    public class UserDepartment : DriveMgr.IDAL.IUserDepartment
    {
        /// <summary>
        /// 设置用户部门（单个用户）
        /// </summary>
        ...
        public bool SetDepartmentBatch(List<Model.UserDepartment> dep_addList, List<Model.UserDepartment> dep_deleteList)
        {
            ...
            try
            {
                DriveMgr.Common.SqlHelper.ExecuteNonQuery(DriveMgr.Common.SqlHelper.connStr, sqlStringListDelete);   //先删
                DriveMgr.Common.SqlHelper.ExecuteNonQuery(DriveMgr.Common.SqlHelper.connStr, sqlStringListAdd);   //再加
                return true;
            }
            catch
            {
