...
{
    public class OutStoreHouseDAL:IOutStoreHouseDAL
    {
        /// <summary>
        /// 增加一条数据
        /// </summary>
        public int AddOutStoreHouse(OutStoreHouseModel model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into tb_OutStoreHouse(");
            strSql.Append("OutSN,OutDate,HandlePerson,Remark,CreatePerson,CreateDate,UpdatePerson,UpdateDate,DeleteMark)");
            strSql.Append(" values (");
            strSql.Append("@OutSN,@OutDate,@HandlePerson,@Remark,@CreatePerson,@CreateDate,@UpdatePerson,@UpdateDate,@DeleteMark)");
            strSql.Append(";select @@IDENTITY");
            SqlParameter[] parameters = {
					new SqlParameter("@OutSN", SqlDbType.VarChar,50),
            ...
            parameters[0].Value = model.OutSN;
        ...
        /// 更新一条数据
        /// </summary>
        public bool UpdateOutStoreHouse(OutStoreHouseModel model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update tb_OutStoreHouse set ");
            strSql.Append("OutSN=@OutSN,");
        ...
        public bool DeleteOutStoreHouse(int id)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("Update tb_OutStoreHouse Set DeleteMark = 1 ");
            strSql.Append(" where Id=@Id");
            SqlParameter[] parameters = {
					new SqlParameter("@Id", SqlDbType.Int,4)
			};
            ...
            //sqlList.Add(@"Update tb_OutStoreHouse Set DeleteMark = 1 Where Id = " + id);
            //sqlList.Add(@"Update tb_OutStoreHouseDetails Set DeleteMark = 1 Where OutStoreHouseID = " + id);
        ...
        /// 得到一个对象实体
        /// </summary>
        public OutStoreHouseModel GetOutStoreHouseModel(int id)
        {

            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 Id,OutSN,OutDate,HandlePerson,Remark,CreatePerson,CreateDate,UpdatePerson,UpdateDate,DeleteMark from tb_OutStoreHouse ");
            strSql.Append(" where Id=@Id");
            SqlParameter[] parameters = {
					new SqlParameter("@Id", SqlDbType.Int,4)
			};
            ...
            OutStoreHouseModel model = new OutStoreHouseModel();
        ...
        /// 得到一个对象实体
        /// </summary>
        private OutStoreHouseModel DataRowToModel(DataRow row)
        {
            OutStoreHouseModel model = new OutStoreHouseModel();
            if (row != null)
            {
                ...
                if (row["OutSN"] != null)
                {
                    model.OutSN = row["OutSN"].ToString();
                }
        ...
        {
            string strSql = "Select Id,OutSN from tb_OutStoreHouse Where DeleteMark = 0 ";
