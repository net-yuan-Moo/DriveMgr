...
namespace DriveMgr.SQLServerDAL
{
    ...
    /// 角色（SQL Server数据库实现）
    /// </summary>
    public class Role : DriveMgr.IDAL.IRole
    {
        /// <summary>
        /// 根据用户id获取用户角色
        /// </summary>
        ...
        public Model.Role GetRoleByRoleName(string roleName)
        {
            ...
            DriveMgr.Model.Role role = null;
            DataTable dt = DriveMgr.Common.SqlHelper.GetDataTable(DriveMgr.Common.SqlHelper.connStr, CommandType.Text, sql, new SqlParameter("@RoleName", roleName));
            if (dt.Rows.Count > 0)
            {
                role = new DriveMgr.Model.Role();
        ...
        public bool DeleteRole(int id)
        {
            ...
            try
            {
                DriveMgr.Common.SqlHelper.ExecuteNonQuery(DriveMgr.Common.SqlHelper.connStr, list);
                return true;
            }
            catch
            {
        ...
        public DataTable GetMenuButtonIdByRoleId(int roleId)
        {
            ...
            return DriveMgr.Common.SqlHelper.GetDataTable(DriveMgr.Common.SqlHelper.connStr, CommandType.Text, sql, new SqlParameter("@roleId", roleId));
        }
        ...
        public bool Authorize(List<Model.RoleMenuButton> role_menu_button_addlist, List<Model.RoleMenuButton> role_menu_button_deletelist)
        {
            ...
            try
            {
                DriveMgr.Common.SqlHelper.ExecuteNonQuery(DriveMgr.Common.SqlHelper.connStr, sqlStringList);   //批量插入（事务）
                return true;
            }
            catch
            {
        ...
        public int GetParentMenuId(int roleId, int menuId)
        {
            ...
            object parentId = DriveMgr.Common.SqlHelper.ExecuteScalar(DriveMgr.Common.SqlHelper.connStr, CommandType.Text, sqlParentId, new SqlParameter("@MenuId", menuId));
            ...
            DataTable dt = DriveMgr.Common.SqlHelper.GetDataTable(DriveMgr.Common.SqlHelper.connStr, CommandType.Text, sqlIfExistsParentRecord, paras);
            if (dt.Rows.Count > 0)
                return -1;  //找到就返回-1
            else
        ...
        public int GetRoleUserCount(int roleId)
        {
            ...
            object count = DriveMgr.Common.SqlHelper.ExecuteScalar(DriveMgr.Common.SqlHelper.connStr, CommandType.Text, sql, new SqlParameter("@roleId", roleId));
        ...
        public DataTable GetPagerRoleUser(int roleId, string order, int pageSize, int pageIndex)
        {
            ...
            return DriveMgr.Common.SqlHelper.GetDataTable(DriveMgr.Common.SqlHelper.connStr, CommandType.Text, strSql.ToString(), new SqlParameter("@roleId", roleId));
        }

        /// <summary>
        /// 把DataRow行转成实体类对象
        /// </summary>
        private void DataRowToModel(DriveMgr.Model.Role model, DataRow dr)
        {
